...
interface StatisticsResult extends MviResult {
  @AutoValue abstract class LoadStatistics implements StatisticsResult {
    ...
    @NonNull static LoadStatistics success(int activeCount, int completedCount) {
      return new AutoValue_StatisticsResult_LoadStatistics(LceStatus.SUCCESS, activeCount,
          completedCount, null);
    }

    @NonNull static LoadStatistics failure(Throwable error) {
      return new AutoValue_StatisticsResult_LoadStatistics(LceStatus.FAILURE, 0, 0, error);
    }

    @NonNull static LoadStatistics inFlight() {
